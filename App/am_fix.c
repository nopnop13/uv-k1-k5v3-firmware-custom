...
const int16_t desired_rssi = (-80 + 160) * 2;
...
{   // called at boot-up
    for (int i = 0; i < 2; i++) {
        gain_table_index[i] = 1;  // re-start with original QS setting
    }
#if !LOOKUP_TABLE
    CreateTable();
#endif
...
void AM_fix_10ms(const unsigned vfo)
{
    ...
    if (diff_dB > 0) {  // decrease gain
        ...
        if (gain_table_index[vfo] != index)
        {
            ...
            hold_counter[vfo] = 50;       // 500ms hold
        }
    }

    if (diff_dB >= -8)                    // 6dB hysterisis (help reduce gain hunting)
        hold_counter[vfo] = 60;           // 600ms hold
    ...
    {   // apply the new settings to the front end registers
        ...
        currentGainDiff = gain_table[1].gain_dB - gain_table[index].gain_dB; // modifié : par rapport au gain max (index 1)
